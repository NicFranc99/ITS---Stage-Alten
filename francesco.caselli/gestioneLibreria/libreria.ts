...
abstract class BaseBook {
    ...
    //Costruttore
    constructor(title: string, author: string, pages: number, publishedYear: number,isbn: number, quantity?: number) {
        this.title = title;
        this.author = author;
        this.pages= pages;
    ...
    //Metodo astratto per stampare le informazioni del libro
...
class Book extends BaseBook {
    ...
    //Costruttore
    constructor(title: string, author: string, pages: number, publishedYear: number, genre: string, publisher: string, isbn: number, quantity?: number) {
...
class Comics extends BaseBook {
    ...
    //Costruttore
    constructor(type: string, title: string, volume: number, author: string, pages: number, color: boolean, publishedYear: number, isbn: number, publisher: string, quantity?: number) {
        super(title, author, pages, publishedYear, isbn, quantity);
        this.publisher = publisher;
        this.color = color;
        this.type = type;
    ...
    //Metodo per stampare le informazioni del fumetto
    override printBookInfo(): string {
...
class Library {
    ...
    add(libroDaAggiungere: BaseBook) {
    ...
    //Metodo per stampare la libreria
    printLibrary() {
    ...
    //Metodo per trovare i libri per autore
    findBooksByAuthor(){
    ...
    //Metodo per trovare i libri per ISBN
    findBooksByIsbn() {
    ...
    //Metodo per trovare i libri per titolo
    findBooksByTitle() {
    ...
    //Metodo per trovare i libri per anno di pubblicazione
    ...
    //Oggetto contenente la data corrente
    ...
    //Metodo per salvare la libreria
    saveToFile(filePath: string) {
        //Converte l'array libreria un json stringa
    ...
    loadFromFile(filePath: string) {
        ...
        if (fs.existsSync(filePath)) {
            ...
            this.library = parsedData.map((item: any) => {
                if (item.type) {
                    return new Comics(
                        item.type,
                        item.title,
...
//Funzione per scrivere un libro
function writeBook() {
    let title: string = readline.question("Inserisci il titolo del libro: ");
    ...
    let author: string = readline.question("Inserisci l'autore del libro: ");
    ...
    let pages: number = Number(readline.question("Inserisci le pagine del libro: "));
    while(isNaN(pages) || pages<=0 || !Number.isInteger(pages) || pages>=10000){
        console.log("Dato non valido, riprova.");
        pages = Number(readline.question("Inserisci le pagine del libro: "));
    }

    let publishedYear: number = Number(readline.question("Inserisci l'anno di pubblicazione del libro, non e' possibile inserire una data antecedente al 1000: "));
    ...
    let isbn: number = Number(readline.question("Inserisci il codice ISBN del libro di 4 cifre: "));
    ...
    let genre: string = readline.question("Inserisci il genere del libro (Avventura, Horror, Romantico): ");
    ...
    let publisher: string = readline.question("Inserisci l'editore del libro: ");
...
//Funzione per scrivere un fumetto
function writeComics() {

    let title: string = readline.question("Inserisci il titolo del fumetto: ");
    while(title===null || title===undefined || title.trim() === "" || title.length>=50){
        console.log("Titolo non valido, riprova.");
        title = readline.question("Inserisci il titolo del fumetto: ");
    }

    let author: string = readline.question("Inserisci l'autore del fumetto: ");
    while(author===null || author===undefined || author.trim() === "" || author.length>=20){
        console.log("Autore non valido, riprova.");
        author = readline.question("Inserisci l'autore del fumetto: ");
    }

    let pages: number = Number(readline.question("Inserisci le pagine del fumetto: "));
    while(isNaN(pages) || pages<=0 || !Number.isInteger(pages) || pages>=10000){
        console.log("Dato non valido, riprova.");
        pages = Number(readline.question("Inserisci le pagine del fumetto: "));
    }

    let publishedYear: number = Number(readline.question("Inserisci l'anno di pubblicazione del fumetto: "));
    ...
    let isbn: number = Number(readline.question("Inserisci il codice ISBN di 4 cifre del fumetto: "));
    while(isNaN(isbn) || isbn<=0 || isbn.toString().length!=4 || !Number.isInteger(isbn)){
        console.log("Dato non valido, riprova.");
        isbn = Number(readline.question("Inserisci il codice ISBN del fumetto: "));
    }

    let publisher: string = readline.question("Inserisci l'editore del fumetto: ");
    while(publisher===null || publisher===undefined || publisher.trim() === "" || publisher.length>=20){
        console.log("Editore non valido, riprova.");
        publisher = readline.question("Inserisci l'editore del fumetto: ");
    }

    const color: boolean = readline.keyInYNStrict("Il fumetto e' a colori? (Y/N): ");

    let type: string = readline.question("Inserisci la tipologia del fumetto (Manga, Manwha, Comics): ");
    ...
    let volume: number = Number(readline.question("Inserisci il volume del fumetto: "));
